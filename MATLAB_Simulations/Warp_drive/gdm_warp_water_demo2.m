...
phi_deg   = 10; % 0            % rotate molecule in-plane (0 = dipole along +x)

% Gaussian "charge" spreads (Å) and amplitudes
% '+' = contraction (H), '−' = dilation (O). Use ~neutral total amplitude.
sigma_H = 0.30;
sigma_O = 0.25;
A_H     = +0.12;           % each H contributes +A_H
A_O     = -0.24;           % O contributes A_O (~ -2*A_H for neutrality)
...
% -------- Place atoms (O at origin; H's around bisector) --------
half = theta_HOH/2;
% Unrotated positions (O at 0)
pO  = [0; 0];
...
% Keep ρ positive (dilation bounded in GDM)
...
hO = plot(pO(1), pO(2), 'wo', 'MarkerFaceColor','w', 'MarkerSize',6);
hH = plot([pH1(1),pH2(1)],[pH1(2),pH2(2)],'w.','MarkerSize',18);
legend([hO,hH], {'O','H'}, 'Location','southoutside');
...
subplot(2,2,4);
% For phi=0, y=0 is the dipole axis. (For general phi, sample along the axis line.)
