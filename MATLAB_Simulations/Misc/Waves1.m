...
% Wave propagation in a 3D deformable cellular space
clc; clear; close all;
...
% Time evolution
for t = 1:T
    % Finite-difference wave equation (3D with spatially varying v)
    for i = 2:Nx-1
        for j = 2:Ny-1
            for k = 2:Nz-1
                % Laplacian term
                ...
                % Update wave equation with spatially varying wave speed
                u_next(i,j,k) = 2*u(i,j,k) - u_prev(i,j,k) + ...
                                (dt^2 * v(i,j,k)^2) * laplacian;
            end
        end
    end
