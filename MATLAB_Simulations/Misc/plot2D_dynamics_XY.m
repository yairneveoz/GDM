function plot2D_dynamics_XY(inputs)
...
for t_ind = 1:length(t)
    ...
    mu_yt = (grid_size/2)*(1 + rotation_R*sin(t(t_ind)));
...
%     q2n = 1/(4*pi)*(RHO2_n - rho_0)./RHO2_n;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [Xp,Yp,q2p,Xn,Yn,q2n] = ...
    dynamicPathXY(X0,Y0,sigma_r,rotation_R,t(t_ind));
    %
        %% (2,2,1):
%     theta = 0:0.1:2*pi;
%     r0p = 1.2*ones(size(theta));
%     r0n = 7.2*ones(size(theta));
%     [x0p,y0p] = pol2cart(theta,r0p);
%     [x0n,y0n] = pol2cart(theta,r0n);
%     ax1 = subplot(2,2,1);
